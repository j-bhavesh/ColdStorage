<div class="card">
    <div class="card-header">
        <div class="d-flex justify-content-between align-items-center">
            <div class="input-group input-group-sm" style="width: 250px;">
                <input type="text" wire:model.live="search" class="form-control" placeholder="Search Seed Distributions...">
            ...
            <div class="tbl-header-actions-bar">
                <button wire:click="create" class="btn btn-primary btn-sm">
                    <i class="fas fa-plus"></i> Add New Distribution
                </button>
    ...
    <div class="card-body p-0">
        <table class="table table-striped table-hover">
            ...
            <tbody>
                @forelse($seedDistributions as $distribution)
                    @php
                    ...
                    <tr>
                        ...
                        <td>{{ $distribution->farmer->farmer_id }}</td>
                        <td>{{ $distribution->farmer->name }}</td>
                        <td>{{ $distribution->farmer->village_name }}</td>
                        <td>{{ $distribution->farmer->user->phone }}</td>
                        <td>{{ $distribution->seedVariety->name }}</td>
                        <td>{{ $distribution->company->name }}</td>
                        <td>{{ $distribution->seedsBooking->bag_quantity }}</td>
                        <td>{{ $receivedBags }}</td>
                        <td>{{ $pendingBags }}</td>
                        ...
                        <td class="text-center">
                            <div class="btn-group btn-group-sm">
                                <button wire:click="edit({{ $distribution->id }})" class="btn btn-info">
                                    <i class="text-white fas fa-edit"></i>
                                </button>
                                <button type="button" 
                                    class="btn btn-danger" 
                                    onclick="if(confirm('Are you sure you want to delete this seed distribution? They will deduct the pending bags from the seeds booking')) { Livewire.find(document.querySelector('[wire\\:id]').getAttribute('wire:id')).call('delete', {{ $distribution->id }}) }">
                ...
                @empty
                    <tr>
                        <td colspan="15" class="text-center py-3">No seed distributions found.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
    ...
    @if($seedDistributions->hasPages())
        <div class="card-footer clearfix">
            {{ $seedDistributions->links() }}
        </div>
    @endif

    @include('livewire.admin.seed-distributions.form', [
        'isOpen' => $isOpen,
        'id' => $id,
        'seedsBookings' => $seedsBookings,
...
$(document).ready(function(){
    function initSelect2() {
        $('#seeds_booking_id').select2({
            ...
            ajax: {
                url: "{{ route('admin.seed-distributions.search') }}",
                dataType: 'json',
                delay: 250, // wait 250ms after typing
                data: function (params) {
                    return {
                        search: params.term, // user input
                        module: 'seed-distributions'
                    };
                },
        ...
        let selectedId = @this.seeds_booking_id ? @this.seeds_booking_id : null;
        let selectedText = @this.seed_booking_name ? @this.seed_booking_name : null;

        if (selectedId && selectedText) {
            let option = new Option(selectedText, selectedId, true, true);
            $('#seeds_booking_id').append(option).trigger('change');
        }

        // Sync back to Livewire when user selects
        $('#seeds_booking_id').off('change.select2').on('change.select2', function (e) {
            @this.set('seeds_booking_id', $(this).val());
        });
    }
    ...
    Livewire.hook('element.updated', (el, component) => {
        if ($(el).find('#seeds_booking_id').length > 0) {
            initSelect2();
        }
        initFinancialYearPicker();
    });
